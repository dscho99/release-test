...
`rc.0` 과 `rc0` 두 표기를 모두 인식하고, 다음 마일스톤을 찾을 때도 둘 다 본다.
즉 `3.2.0-rc.0` 다음으로 `3.2.0-rc.1` 을 찾되 없으면 `3.2.0-rc1` 도 찾아보고,
**정말 없을 때만 새로 만든다** (표기가 갈려서 마일스톤이 중복 생성되는 걸 막는다).

새로 만들 때는 현재 마일스톤의 표기를 따른다. 그래도 레포 안에서 표기를 하나로
통일해 두는 편이 낫다.
...
| `RC_PATTERN` | `^(.*)rc(\.?)([0-9]+)$` | 대상 제목 패턴. 그룹1=접두사, 2=구분자, 3=rc 번호 |
